...
def setCliParameters():
    ...
    try:
        # Parse CLI arguments
        opts, args = getopt.getopt(sys.argv[1:], 'hs:u:p:a:i:d:z:x:y:o:v', ['help','sessionfile','username','password','app','sessionid','database','pod','client_id','client_secret','orgid'])
    except getopt.GetoptError:
        print 'apexResultsRest.py -s <sessionfile> -i <sessionid> -u <username> -p <password> -a <application> -d <database> -z <pod> -x <client_id> -y <client_secret> -i <org_id>'
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
                print 'apexResultsRest.py -s <sessionfile> -i <sessionid> -u <username> -p <password> -a <application> -d <database>  -z <pod> -x <client_id> -y <client_secret> -i <org_id>'
...
def enrichDataFrame(df):
    ...
    if type(df['ApexTestRunResult'][0]) is dict:
        ...
        if 'TestTimestamp' in df.columns:
            df['JobDate'] = pd.to_datetime(df.TestTimestamp).dt.strftime('%m-%d-%Y')
            df.JobDate.replace('NaT','', inplace=True)
    ...
    if 'EndTime' in df.columns:
        cols.insert(25, cols.pop(cols.index('EndTime')))
    df = df.ix[:, cols]
    # Add App Name column
    ...
    # Add Org ID column 
    df['OrgId'] = pd.Series(orgid, index=df.index)
    # Delete JSON object
    del df['ApexClass']
    del df['attributes']
...
# Org ID used only for reporting
orgid = ''
# Apex query
